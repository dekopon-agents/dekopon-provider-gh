#!/usr/bin/env bash
# Build gh-provider.wasm reproducibly.
#
# This is a self-contained port of dekopon's examples/providers/build-component.sh. The provider
# was built by that shared driver while it lived in that tree, and the harness comes with it:
# provenance attestation proves who built an artifact, reproducibility proves what it was built
# from, and a release should not ship without both.
#
...
# provider, and keeping it means this harness reproduces the component the dekopon tree shipped
# byte for byte when fed the same source. Do not change this salt casually.
set -euo pipefail
...
core="$target_root/wasm32-unknown-unknown/release/dekopon_gh_provider.wasm"
component=${1:-"$root/gh-provider.wasm"}
